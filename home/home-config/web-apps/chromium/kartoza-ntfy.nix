{
  ...
  siteUrl = "https://ntfy.sh/app";
  appName = "Kartoza Ntfy";
  iconName = "kartoza-ntfy.svg";
  iconPath = "${config.home.homeDirectory}/.local/share/icons/${iconName}";
in {
  home.file."kartoza_ntfy_image" = {
    source = ./${iconName};
    target = iconPath;
  };
  home.packages = with pkgs; let
    myKartozaNtfyApp = makeDesktopItem {
      name = appName;
      desktopName = appName;
      genericName = appName;
      exec = ''
