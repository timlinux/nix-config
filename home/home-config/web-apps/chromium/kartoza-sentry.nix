{
  ...
  siteUrl = "https://sentry.do.kartoza.com";
  appName = "Kartoza Sentry";
  iconName = "kartoza-sentry.svg";
  iconPath = "${config.home.homeDirectory}/.local/share/icons/${iconName}";
in {
  home.file."kartoza_sentry_image" = {
    source = ./${iconName};
    target = iconPath;
  };
  home.packages = with pkgs; let
    myKartozaSentryApp = makeDesktopItem {
      name = appName;
      desktopName = appName;
      genericName = appName;
      exec = ''
      ...
      startupWMClass = "sentry.do.kartoza.com";      
    };
  in [myKartozaSentryApp];
}
